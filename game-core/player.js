...
function Tail(player, sdata) {
  ...
  defineInstanceMethods(this, data, addTail, hitsTail, fillTail, renderTail,
    reposition, serialData);
...
function floodFill(data, grid, row, col, been) {
  ...
  var start = [row, col];
  if (grid.isOutOfBounds(row, col) || been.get(row, col) || onTail(start) ||
    grid.get(row, col) === data.player)
...
function Player(grid, sdata) {
  ...
  var base;
  if (sdata.base)
    base = this.baseColor = sdata.base instanceof Color ? sdata.base : Color.fromData(
      sdata.base);
  else {
    var hue = Math.random();
  ...
  defineAccessorProperties(this, data, "currentHeading", "dead", "name", "num",
    "posX", "posY", "grid", "tail", "waitLag");
...
function calcRow(data) {
  return nearestInteger(data.currentHeading === 2 /*DOWN*/ , data.posY /
    CELL_WIDTH);
}

function calcCol(data) {
  return nearestInteger(data.currentHeading === 1 /*RIGHT*/ , data.posX /
    CELL_WIDTH);
}
...
Player.prototype.render = function(ctx, fade) {
  ...
  var grd = ctx.createRadialGradient(this.posX + mid, this.posY + mid -
    SHADOW_OFFSET, 1,
  ...
  ctx.fillStyle = grd;
  ctx.fillRect(this.posX - 1, this.posY - SHADOW_OFFSET, CELL_WIDTH + 2,
    CELL_WIDTH);
